...
describe('getSearchBuilderSql', () => {
  ...
  describe('handle single criterion', () => {
    it("AND logic", () => {
      ...
      assert.equal(adapter.getSearchBuilderSql(ex), `(("Title" LIKE '%q%') AND True)`);
    });
    it("OR logic", () => {
      const ex = { "criteria": [ { "condition": "contains", "data": "Title", "origData": "Title", "type": "string", "value": [ "q" ], "value1": "q" } ], "logic": "OR" };
      assert.equal(adapter.getSearchBuilderSql(ex), `(("Title" LIKE '%q%') OR False)`);
    });
  });
  ...
  describe('handle multiple criteria (2nd level)', () => {
    ...
    it("AND logic", () => {
      assert.equal(adapter.getSearchBuilderSql({logic: "AND", ...ex}), `(("Department" = 'english') AND ("Title" LIKE '%bovary%'))`);
    });
    it("OR logic", () => {
      assert.equal(adapter.getSearchBuilderSql({logic: "OR", ...ex}), `(("Department" = 'english') OR ("Title" LIKE '%bovary%'))`);
    });
  });
  ...
  describe('handle nested criteria (3rd level)', () => {
    it("Outer OR, inner ANDs", () => {
      ...
      assert.equal(adapter.getSearchBuilderSql(nested), `((("Department" = 'Photography') AND ("LastName" = 'Goldin')) OR (("Department" = 'Print') AND ("LastName" = 'Goya')))`);
    });
    it("Outer OR, ONE inner AND", () => {
      ...
      assert.equal(adapter.getSearchBuilderSql(nested), `((("Department" = 'english') AND ("Title" LIKE '%spanish%')) OR ("Department" = 'spanish'))`);
    });
    it("Outer AND, ONE inner OR", () => {
      ...
      assert.equal(adapter.getSearchBuilderSql(nested), `((("Department" = 'english') OR ("Title" LIKE '%spanish%')) AND ("Department" = 'spanish'))`);
    });
